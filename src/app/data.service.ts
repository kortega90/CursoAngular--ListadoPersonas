...
export class DataService {
    ...
    cargarPersonas(){
        ...
        return this.httpClient.get('https://listado-persona-afc62-default-rtdb.firebaseio.com/datos.json?auth=' + token);
    }

    //Guarda todo el arreglo de personas 
    guardarPersonas(personas: Persona[]) {
        const token = this.loginService.getIdToken();
        this.httpClient.put('https://listado-persona-afc62-default-rtdb.firebaseio.com/datos.json?auth=' + token, personas)
            .subscribe(
                (response) => {
    ...
    modificarPersona(index:number, persona: Persona){
        let url: string;
        const token = this.loginService.getIdToken();
        url = 'https://listado-persona-afc62-default-rtdb.firebaseio.com' + '/datos/' + index + '.json?auth=' + token;
